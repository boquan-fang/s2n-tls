...
/* kTLS is not supported on Windows. Provide minimal stubs. */
#include "error/s2n_errno.h"
#include "utils/s2n_safety.h"

bool s2n_ktls_is_supported_on_platform() { return false; }
S2N_RESULT s2n_ktls_get_file_descriptor(struct s2n_connection *conn, s2n_ktls_mode ktls_mode, int *fd) { RESULT_BAIL(S2N_ERR_KTLS_UNSUPPORTED_PLATFORM); }
int s2n_ktls_send_cb(void *io_context, const uint8_t *buf, uint32_t len) { POSIX_BAIL(S2N_ERR_KTLS_UNSUPPORTED_PLATFORM); }
S2N_RESULT s2n_ktls_sendmsg(void *io_context, uint8_t record_type, const struct iovec *msg_iov, size_t msg_iovlen, s2n_blocked_status *blocked, size_t *bytes_written) { RESULT_BAIL(S2N_ERR_KTLS_UNSUPPORTED_PLATFORM); }
S2N_RESULT s2n_ktls_recvmsg(void *io_context, uint8_t *record_type, uint8_t *buf, size_t buf_len, s2n_blocked_status *blocked, size_t *bytes_read) { RESULT_BAIL(S2N_ERR_KTLS_UNSUPPORTED_PLATFORM); }
ssize_t s2n_ktls_sendv_with_offset(struct s2n_connection *conn, const struct iovec *bufs, ssize_t count, ssize_t offs, s2n_blocked_status *blocked) { POSIX_BAIL(S2N_ERR_KTLS_UNSUPPORTED_PLATFORM); }
int s2n_ktls_record_writev(struct s2n_connection *conn, uint8_t content_type, const struct iovec *in, int in_count, size_t offs, size_t to_write) { POSIX_BAIL(S2N_ERR_KTLS_UNSUPPORTED_PLATFORM); }
int s2n_ktls_read_full_record(struct s2n_connection *conn, uint8_t *record_type) { POSIX_BAIL(S2N_ERR_KTLS_UNSUPPORTED_PLATFORM); }
S2N_RESULT s2n_ktls_key_update_send(struct s2n_connection *conn, size_t bytes_requested) { RESULT_BAIL(S2N_ERR_KTLS_UNSUPPORTED_PLATFORM); }
S2N_RESULT s2n_ktls_key_update_process(struct s2n_connection *conn) { RESULT_BAIL(S2N_ERR_KTLS_UNSUPPORTED_PLATFORM); }
S2N_RESULT s2n_ktls_set_estimated_sequence_number(struct s2n_connection *conn, size_t bytes_written) { RESULT_BAIL(S2N_ERR_KTLS_UNSUPPORTED_PLATFORM); }
S2N_RESULT s2n_ktls_check_estimated_record_limit(struct s2n_connection *conn, size_t bytes_requested) { RESULT_BAIL(S2N_ERR_KTLS_UNSUPPORTED_PLATFORM); }
int s2n_connection_ktls_enable_send(struct s2n_connection *conn) { POSIX_BAIL(S2N_ERR_KTLS_UNSUPPORTED_PLATFORM); }
int s2n_connection_ktls_enable_recv(struct s2n_connection *conn) { POSIX_BAIL(S2N_ERR_KTLS_UNSUPPORTED_PLATFORM); }
int s2n_config_ktls_enable_unsafe_tls13(struct s2n_config *config) { POSIX_BAIL(S2N_ERR_KTLS_UNSUPPORTED_PLATFORM); }
